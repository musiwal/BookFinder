...
export class Book {
    ...
    public async updateRating(req: Request, res: Response): Promise<void> {
        ...
        const bookRating: IBook = await BookModel.findOneAndUpdate(
            {_id: bookId }, { $inc: { rating: rating } }, { new: true }
        ) as IBook;
        res.status(HTTP_STATUS.CREATED).json({ message: 'Rating updated', book: bookRating });
    }
